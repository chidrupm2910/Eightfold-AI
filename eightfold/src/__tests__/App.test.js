...
describe('MOVIE INPUT TESTS', () => {
    ...
    it('Should Render One Input Component for Movie Search', () => {
    ...
    it('Checking Dispatched actions in the component when a list item is clicked', () => {
        ...
        }, { type: actionTypes.UPDATE_SELECTED_MOVIE_LIST, selectedMovieList: [...initialState.movies.selectedMovieList, 'Movie 5'] }]
        expect(store.getActions()).toEqual(expectedAction);
    ...
    it('Number  of posters displayed need to be same as number of List Items', () => {
        const images = wrapper.find('.list-item img');
        expect(images.length).toBe(initialState.movies.dropdownMovieList.length);
    })
